...
namespace Algorithm
{
    ...
    class Player
    {
        ...
        void Astar()
        {
            ...
            // 시작점 발견 (예약 진행)
            open[PosX, PosY] = Math.Abs(_board.DestY - PosY) + Math.Abs(_board.DestX - PosX);
        ...
        {
            int y = _board.DestY;
            int x = _board.DestX; // 끝점에서 시작점까지 거슬러서 진행
            while (parent[y, x].Y != y || parent[y, x].X != x) // 부모와 자식의 좌표가 같으면(시작점이라면) 루프 종료
            {
        ...
        const int MOVE_TICK = 50; // 0.05초마다 움직일 것
        int _sumTick = 0;
        int _lastIndex = 0;
        public void Update(int deltaTick)
        {
            ...
            _sumTick += deltaTick;
            if (_sumTick >= MOVE_TICK) // 지금까지 지난 시간이 0.05초를 넘긴다면 움직인다.
            {
                _sumTick = 0;
